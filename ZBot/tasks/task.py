...
@app.task(bind=True, max_retries=3, acks_late=True, default_retry_delay=300, ignore_result=True, queue="default")
...
async def _run_memory_operations():
    ...
    try:
        obsolete_ok = await daily_memory_store.obsolete_daily_memory(
            decay_rate=config.decay_rate, obsolete_score_threshold=config.obsolete_score_threshold
        )
        logger.info(f"日常记忆清理结果: {'成功' if obsolete_ok else '失败'}")

        # 2. 晋升并写入长期记忆
        evolve_data = await daily_memory_store.evolve_daily_memory(
            decay_rate=config.decay_rate, evolve_score_threshold=config.evolve_score_threshold
        )
        write_ok = await long_term_memory_store.write_long_term_memory(
            provider=provider, model=config.model, filtered_daily_memory=evolve_data
        )
