...
from ZBot.config.loader import get_path_config  # 获取配置文件路径的函数


def get_runtime_subdir(name: str) -> Path:
    """返回根数据文件夹下的指定子文件夹路径。

    例如 get_runtime_subdir("cron") 会返回 ~/.ZBot/cron，
    用于存放定时任务等运行时数据。
...
def get_workspace_path(workspace: str | None = None) -> Path:
    ...
    参数：
        workspace: 如果显式传入了工作区路径，则直接返回；
                   如果为 None，则使用默认路径 ~/.ZBot/workspace
...
def get_cli_history_path() -> Path:
    ...
    返回：
        历史记录文件的 Path 对象（~/.ZBot/history/cli_history）
    """
    return Path.home() / ".ZBot" / "history" / "cli_history"
