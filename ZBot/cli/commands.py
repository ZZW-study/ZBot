...
from ZBot.utils.helpers import sync_workspace_templates                  # 工作区模板同步
...
def version_callback(value: bool) -> None:
    ...
    """
    if value:
        console.print(f"{__logo__} ZBot v{__version__}")  # 打印版本信息
...
def onboard():
    ...
    """
    # 延迟导入配置相关函数
    from ZBot.config.loader import get_path_config, load_config, save_config
    ...
    console.print('  3. 开始对话： [cyan]ZBot agent -m "你好！"[/cyan]')
...
def _make_provider(config: Config):
    ...
    # 延迟导入（避免循环依赖）
    from ZBot.config.loader import get_path_config
