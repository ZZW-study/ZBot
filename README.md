...
- Python 3.13.x（见 `pyproject.toml` 与 `.python-version`）
- `uv` 0.11+（推荐，使用 `uv.lock` 创建虚拟环境）
...
Windows 下如果 `uv run` 因全局缓存目录权限报错，可先在当前 PowerShell 会话执行：

```powershell
$env:UV_CACHE_DIR = "$PWD\.uv-cache"
```

下文默认使用 `uv run python -m ZBot ...`。如果你已经激活 `.venv`，也可以直接执行 `python -m ZBot ...`。
